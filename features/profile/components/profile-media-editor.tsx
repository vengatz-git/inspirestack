...
export function ProfileMediaEditor({
  ...
  async function handleConfirm() {
    ...
    try {
      const file = await createCroppedImage(imageUrl, croppedAreaPixels);
    ...
    >
      <div className="bg-background flex w-full max-w-3xl flex-col overflow-hidden rounded-2xl shadow-2xl">
        ...
        <div className="space-y-4 border-t p-4">
          <label htmlFor="profile-media-zoom" className="text-sm font-medium">
            Zoom
          </label>
...
async function createCroppedImage(imageUrl: string, crop: Area): Promise<File> {
