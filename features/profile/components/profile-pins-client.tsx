...
export function ProfilePinsClient({
  username,
  initialPins,
  initialCursor,
  ...
  const loadMore = useCallback(async () => {
    ...
    try {
      const response = await fetch(
        `/api/profile/${encodeURIComponent(username)}/pins?cursor=${encodeURIComponent(nextCursor)}`,
      );

      if (!response.ok) {
        throw new Error("Failed to load more pins.");
      }
